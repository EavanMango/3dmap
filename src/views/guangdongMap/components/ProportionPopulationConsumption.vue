<template>
  <div class="right-card" ref="cardRef">
    <m-card title="人群消费占比" :height="height">
...
const handleResize = () => {
  height.value = cardRef.value.offsetHeight - 2;
};
const { destroy } = useThrottle(handleResize);
onMounted(() => {
  nextTick(() => {
    handleResize();
  });
...
.population-proportion {
  ...
  &-chart {
    ...
    height: 100%;
    margin-left: 15px;
    background: url("~@/assets/images/pie/pie-zs-bg.png") left center no-repeat;
